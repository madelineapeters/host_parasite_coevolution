...
class population
{
    public:
        //Variables
        int *state; //state of deme (four element vector of host type 1 and 2 and parasite 1 and 2)
...
void metapopulation::calculateRates()
{
    ...
    //for (int e=0;e<nE;e++) out_Test<<ratesVec[e]<<",";
    //out_Test<<endl;
    //out_Test.close();
}
...
void metapopulation::simulation(char* argv1, int rep)
{
    ...
    for(int s=0;s<4;s++) out_Data<<demes[1].state[s]<<",";
    out_Data<<endl;
