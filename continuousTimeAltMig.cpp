...
void metapopulation::initializeEventsMtx()
{
    ...
    //Host demography (4)
        ...
        s=1;
        //Host migration (8)
        for(int i=0;i<2;i++) //host that migrations from pop u to pop u'
        {
            for(int k=0;k<2;k++) //host that is born in pop u
            {
                for(int m=0;m<2;m++) //host that dies in pop u'
                ...
                //Host H[k,u] born in pop u
                eventsMtx[ct][u*4+(s-1)*2+k]++;
                //Host H[m,v] dies in pop u'
        ...
        for(int j=0;j<2;j++)
        {
            for(int l=0;l<2;l++)
            {
                ...
                //Parasite P[l,u] born in pop u
                eventsMtx[ct][u*4+(s-1)*2+l]++;
                //Parasite P[n,v] dies in pop u'
                eventsMtx[ct][((u+1)%2)*4+(s-1)*2+n]--;
                ct++;
                }
            }
        }
