...
int main(int argc, char**argv) //return integer (int), first argument is integer called argc, second is array of characters called argv
{
    ...
    sprintf(filename,"./Data_Island_%s",argv[1]);
    mkdir(filename,S_IRWXU);
    sprintf(filename,"./Data_Island_%s/parameters_%s.csv",argv[1],argv[1]); //Makes the input file name
...
void input(metapopulation *metaPopPtr,char* argv1)
{
    ...
    ifstream inFile;
    sprintf(filename,"./inputMut_%s.txt",argv1); //Makes the input file name
...
void metapopulation::calculateRates()
{
    ...
    for (int u=0;u<2;u++){
        ...
        for(int i=0;i<2;i++) //host that migrates from pop u
        {
            ...
            {
                ratesVec[ct]=D*phi*(double)demes[u].state[(s-1)*2+i]*((double)demes[u].state[(s-1)*2+k]/(double)kappaH);
                rateTotal+=ratesVec[ct];
                ct++;
            }
        }
        //Host migration in from mainland
        for(int i=0;i<2;i++) //host that migrates into pop u
        {
            for(int k=0;k<2;k++) //host that dies following migration
            {
                ratesVec[ct]=D*phi*0.5*((double)demes[u].state[(s-1)*2+k]/(double)kappaH);
                rateTotal+=ratesVec[ct];
                ct++;
            }
        }

        //Parasite migration out
        s=2;
        for(int j=0;j<2;j++) //parasite that migrates from pop u
        {
            for(int l=0;l<2;l++) //parasite that is born in pop u
            {
                ...
                ratesVec[ct]=D*omega*(double)demes[u].state[(s-1)*2+j]*((double)demes[u].state[(s-1)*2+l]/(double)kappaP);
                rateTotal+=ratesVec[ct];
                ct++;
            }
        }
        for(int j=0;j<2;j++) //parasite migrates into pop u
        {
            for(int l=0;l<2;l++) //parasite that dies following migration in pop u
            {
                //omega P[j,u] P[l,v]/kappaP P[n,u]/kappaP
                ratesVec[ct]=D*omega*0.5*((double)demes[u].state[(s-1)*2+l]/(double)kappaP);
                rateTotal+=ratesVec[ct];
                ct++;
            }
        }
...
void metapopulation::initializeEventsMtx()
{
    ...
    for (int u=0;u<2;u++){
        ...
        for(int i=0;i<2;i++)
        {
            for(int k=0;k<2;k++)
            {
                    //Host of type i moves from pop mainland to pop u
                    eventsMtx[ct][u*4+(s-1)*2+i]++;
                    //Host of type k dies following migration
                    eventsMtx[ct][u*4+(s-1)*2+k]--;
                    ct++;
            }
        }
        ...
        //Parasite migration in (4)
        for(int j=0;j<2;j++)
        {
            for(int l=0;l<2;l++)
            {
                    //Parasite of type j moves from mainland to pop u
                    eventsMtx[ct][u*4+(s-1)*2+j]++;
                    //Parasite of type l dies following migration
                    eventsMtx[ct][u*4+(s-1)*2+l]--;
                    ct++;
            }
        }
...
{
    sprintf(filename,"./Data_Island_%s/data_island_%s_%d.csv",argv1,argv1,rep);
    out_Data.open(filename);

    sprintf(filename,"./Data_Island_%s/dataN_island_%s_%d.csv",argv1,argv1,rep);
...
void metapopulation::neutralEvent(int event)
{
    ...
    else if (e2 < 28)//host migration out
    {
    ...
    else if (e2 < 32){//host migration in from mainland
        
        //Choose host to move into of focal population
        if(randH<((double)demes[p].stateN[0]/(double)kappaH)){
            temp[p*4+0]++;
        } else {
            temp[p*4+1]++;
        ...
        //Choose host to die in focal population
        if(randH2<((double)demes[p].stateN[0]/(double)kappaH)){
            temp[p*4+0]--;
        } else {
            temp[p*4+1]--;
    ...
    else if (e2 < 36)//parasite migration
    {
    ...
    {
     //Choose parasite to move into focal population
        if(randP<((double)demes[p].stateN[2]/(double)kappaP)){
            temp[p*4+2]++;
        } else {
            temp[p*4+3]++;
